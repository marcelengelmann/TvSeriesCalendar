    ...
    class Scheduler
    {
        ...
        public ITaskFolder CreatePath(String Path)
        {
            ...
            for (i = 0; i < parts.Length; i++)
            {
                try
                {
                    if (folder.GetFolder(parts[i]) != null)
                        continue;
                    folder.CreateFolder(parts[i]);
                }
                catch (Exception)
                {
                } // bei jedem bereits existierenden Pfad, daher einfach ignorieren
            }
        ...
        public void AddAutorunTask(String Name, String PathFilename, String Username, String Password)
        {
            ...
            exec_action.Path =
                "\"" + PathFilename +
                "\""; // tatsächlich PathFilename in Hochkommata, Bezeichner "Path" ist schlicht falsch
            exec_action.WorkingDirectory =
                Path.GetDirectoryName(PathFilename); // darf nicht in Hochkommata eingeschlossen werden
            ...
            registerd_task = folder.RegisterTaskDefinition(Name, definition, 6, Username, Password,
